...
| 317 | 数据库导入列丢失Bug修复 | 前端 Bug修复 | 2026-06-15 | Assistant | 修复从数据库导入时表创建成功但列数据丢失的核心缺陷：1.根因分析：App.tsx的handleDatabaseImport函数缺少对tableInfo.columns的防御性检查，当columns为undefined或空数组时跳过列创建导致空表；2.添加完整的数据验证日志系统（[DatabaseImport]前缀），便于追踪导入数据结构；3.实现防御性编程：检查表信息有效性、验证columns字段存在且为数组、使用tableInfo.columns || []安全访问；4.改进表查找机制：增加重试逻辑（首次失败后等待200ms再次查找），提高异步状态更新可靠性；5.增强错误处理：为每个列创建操作添加try-catch，单个列失败不中断整个表的导入；6.优化性能：缩短列创建延迟从TIME.TABLE_CREATE_DELAY_MS到50ms；7.详细的结果反馈：显示成功/失败表数量和创建的列总数，替代原来的简单提示；8.TypeScript编译零错误、Vite构建成功
| 318 | 数据库导入列丢失根因修复(Zustand状态同步) | 前端 Bug修复 | 2026-06-15 | Assistant | **彻底修复**数据库导入后"表创建成功但新建字段0"的Bug。1.根因分析：handleDatabaseImport中tables变量来自useAppStore() React hooks解构(闭包)，createTable()内部set()更新了Zustand store但不会立即更新闭包中的tables变量（需等待React重新渲染），导致tables.find()始终找不到刚创建的表；2.核心修复：所有tables查找从`tables.find()`改为`useAppStore.getState().tables.find()`，直接从Zustand store获取最新状态，完全绕过React渲染周期；3.新增表定位策略：记录创建前表数量→名称查找失败后通过数量变化定位最后一张表→300ms延迟最终重试，三层保障；4.列查找同样使用storeGet()获取最新状态；5.失败时输出当前项目所有表列表辅助调试；6.移除useCallback依赖中的tables（不再需要）；7.TypeScript编译零错误(exit code 0)、Vite构建成功(40.44s) | — | 分支 ↔ 版本/画布联动 | 全栈 功能增强 | 2026-05-18 | Assistant | P1 集成增强：1.后端 versionService.create() 自动查询活跃分支并在创建版本时填充 branchId；2.前端 BranchManager.onBranchChange 回调调用 selectProject() 实现切换分支后自动刷新画布数据（重新加载 tables/columns/indexes）
...
| 388 | E-R图生成功能增强（工业级升级） | 前端 导出功能 | 2026-06-12 | Assistant | 全面增强E-R图导出功能（PNG/SVG）：1) 主键(PK)标识系统：橙色左侧边栏+🔑图标+加粗字体；2) 外键(FK)标识系统：绿色左侧边栏+🔗图标+特殊字体；3) 字段约束显示：NOT NULL、UNIQUE、AUTO_INC自动识别并标注；4) 关系类型可视化区分：1:1虚线(8,4)、1:N实线、M:N点线(4,4)+关系标签；5) 表/字段注释完整支持：表头注释半透明白色、字段注释灰色小字；6) 专业SVG样式：阴影滤镜(drop-shadow)、6px圆角、交替行背景色(奇偶行区分)；7) 图例说明面板：底部固定图例包含PK/FK/1:1/1:N/M:N说明；8) XSS安全防护：escapeXml函数转义所有用户输入内容；9) 自适应布局算法优化：支持表注释动态高度、自动换行、智能画布尺寸计算；10) 配置常量结构化重构：SVG_EXPORT对象包含colors/fontSize/尺寸等完整配置。TypeScript编译零错误、Vite构建成功、后端健康检查通过、前端HTTP 200正常访问 |
| 389 | E-R图标准格式修复（Chen Notation） | 前端 导出功能 | 2026-06-12 | Assistant | **核心修复**：将导出图从类图(Class Diagram)风格改为标准E-R图(Entity-Relationship Diagram, Chen Notation)格式。**问题根因**：原实现使用简单直线连接表节点，缺少E-R图核心元素菱形关系节点，导致看起来像UML类图而非数据库E-R图。**修复内容**：1) 新增ER_CONFIG配置对象（替代SVG_EXPORT），包含实体/关系/属性的完整E-R图规范参数；2) 实体(Entity)用蓝色矩形表示（#2563eb边框+渐变头部）；3) 关系(Relationship)用橙色菱形(Diamond)表示（#ea580c边框+渐变填充），位于两实体中间；4) 连线结构改为标准E-R格式：源实体→菱形→目标实体；5) 基数(Cardinality)标注在连线端点处（圆形标签显示1/N/M），符合Chen notation规范；6) 主键(PK)用下划线+加粗+深橙色标识，外键(FK)用斜体+深绿色标识，主键外键兼有用紫色斜体下划线；7) 标题区域显示"E-R Diagram: 项目名"和"Entity-Relationship Diagram (Chen Notation)"副标题；8) 图例更新为E-R图符号说明（实体矩形/关系菱形/主键/外键/基数）；9) 属性(Attribute)在实体内列表显示，带PK/FK前缀和约束缩写(NN/UQ/AI)；10) XSS安全防护保留escapeXml函数。**验证结果**：TypeScript编译零错误、Vite构建成功(24.32s)、VS Code诊断零错误、前端HTTP 200正常运行、项目完整可运行 |

---

## 变更记录
- 2026-06-12: 添加E-R图标准格式修复（Chen Notation）归档（389）：从类图风格改为标准E-R图格式，新增菱形关系节点、标准连线结构、基数标注、专业E-R图符号系统
- 2026-06-12: 添加E-R图生成功能工业级增强归档（388）：主键/外键标识系统、字段约束显示、关系类型区分、注释支持、专业SVG样式、图例面板、XSS防护、自适应布局
